...
const MultipleColumnsBtn = () => {
    ...
    const fillingColor = getFillingColor(isDarkMode, singleColumn);

    return (
        <>
            {/* prettier-ignore */}
            <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={() => setSingleColumn(false)}>
                <rect className="multipleColumnsBtn__background" width="40" height="40" rx="10" 
                      fill={isDarkMode ? "#4A4A4A" : "#E1E1E1"} />
...
const SingleColumnBtn = () => {
    ...
    return (
        <>
            {/* prettier-ignore */}
            <svg style={{transition: "all 0.2s"}}  id="singleColumnBtn" width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={() => setSingleColumn(true)}>
                <rect className="singleColumnBtn__background" width="40" height="40" rx="10" 
                      fill={isDarkMode ? "#4A4A4A" : "#E1E1E1"} />
...
function getFillingColor(isDarkMode, singleColumn) {
    let fillingColor;
    if (isDarkMode && !singleColumn) {
        fillingColor = '#FFFDF5';
    } else if (isDarkMode && singleColumn) {
        fillingColor = '#707070';
    } else if (!isDarkMode && !singleColumn) {
        fillingColor = '#515151';
    } else {
